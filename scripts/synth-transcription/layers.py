...
for k in range(K):
    for s, e in runs(slow[:, k], 0.30, 0.18, 2.0, 1.0):
        notes.append(dict(layer='slow', start=round(s / fps, 2), end=round(e / fps, 2), midi=21 + k,
    ...
    for s, e in runs(x, 0.5, 0.25, 0.2, 0.08):
        ...
        if ons < 0.3 and pre > 0.2:
            continue
        notes.append(dict(layer='fast', start=round(s / fps, 3), end=round(e / fps, 3), midi=21 + k,
...
if a.png:
    ...
    p = 0.0; pg = 0; end = F / fps
    while p < end:
        q = min(end, p + a.page); fr = slice(int(p * fps), int(q * fps))
