...
// ── Wave-4 helpers: data-driven cues ───────────────────────────────────

/**
 * Chord-cycle cue: pads + bass looping a progression, optional lead melody.
 * prog: [{ chord: [names], bass: name|null }], melody: [[note, at, beats, vel]].
 */
const chordCue = ({
  id, name, bpm, seed, prog, passes = 2, barBeats = 4,
  padVel = 0.5, bassVel = 0.6, melody = []
}) => {
  const pads = new Part({ bpm, humanize: 0.015, seed });
  const bass = new Part({ bpm, seed: seed + 1 });
  const lead = new Part({ bpm, humanize: 0.02, seed: seed + 2 });
  const cycleBeats = prog.length * barBeats;
  for (let pass = 0; pass < passes; pass++) {
    prog.forEach((bar, i) => {
      const at = pass * cycleBeats + i * barBeats;
      if (bar.chord?.length) pads.chord(bar.chord, at, barBeats * 1.02, padVel);
      if (bar.bass) bass.add(bar.bass, at, barBeats, bassVel);
    });
  }
  melody.forEach(([note, at, beats, vel]) => lead.add(note, at, beats, vel));
  const parts = [pads, bass, lead].filter((part) => part.notes.length > 0);
  writeMidi({ id, name, bpm, parts });
};

/**
 * Cell-groove cue: step-sequenced bars from named cells plus optional
 * per-bar bass and a free extra track (leads, stabs, bells).
 * plan: [{ cell, bass?, bassBeats?, bassVel? }] — one entry per bar.
 */
const cellCue = ({
  id, name, bpm, seed, cells, plan, stepBeats = 0.25, noteBeats = 0.22,
  extra = []
}) => {
  const seq = new Part({ bpm, seed });
  const bass = new Part({ bpm, seed: seed + 1 });
  const extras = new Part({ bpm, humanize: 0.012, seed: seed + 2 });
  plan.forEach((entry, barIndex) => {
    const start = barIndex * 4;
    (cells[entry.cell] || []).forEach(([step, note, vel]) => {
      seq.add(note, start + step * stepBeats, noteBeats, vel);
    });
    if (entry.bass) bass.add(entry.bass, start, entry.bassBeats ?? 4, entry.bassVel ?? 0.66);
  });
  extra.forEach(([note, at, beats, vel]) => extras.add(note, at, beats, vel));
  const parts = [seq, bass, extras].filter((part) => part.notes.length > 0);
  writeMidi({ id, name, bpm, parts });
};

// ── Wave-4 pieces (25) ─────────────────────────────────────────────────

const wave4ChordCues = () => {
  chordCue({
    id: 'original-neon-cathedral', name: 'Neon Cathedral (Anthem)', bpm: 72, seed: 401,
    prog: [
      { chord: ['A2', 'E3', 'A3', 'C4'], bass: 'A1' }, { chord: ['F2', 'C3', 'A3', 'C4'], bass: 'F1' },
      { chord: ['C3', 'G3', 'C4', 'E4'], bass: 'C2' }, { chord: ['G2', 'D3', 'B3', 'D4'], bass: 'G1' },
      { chord: ['A2', 'E3', 'A3', 'C4'], bass: 'A1' }, { chord: ['F2', 'C3', 'F3', 'A3'], bass: 'F1' },
      { chord: ['D3', 'A3', 'D4', 'F4'], bass: 'D2' }, { chord: ['E3', 'G#3', 'B3', 'E4'], bass: 'E2' }
    ],
    melody: [
      ['E5', 8, 3, 0.7], ['D5', 11, 1, 0.62], ['C5', 12, 2, 0.66], ['B4', 14, 2, 0.6],
      ['A4', 16, 4, 0.66], ['G4', 20, 2, 0.6], ['A4', 22, 2, 0.62], ['B4', 24, 3, 0.66],
      ['E4', 27, 1, 0.54], ['A4', 28, 4, 0.64],
      ['C5', 40, 3, 0.72], ['D5', 43, 1, 0.66], ['E5', 44, 4, 0.76], ['F5', 48, 2, 0.72],
      ['E5', 50, 2, 0.68], ['D5', 52, 3, 0.66], ['C5', 55, 1, 0.6], ['B4', 56, 2, 0.62],
      ['C5', 58, 2, 0.64], ['A4', 60, 4, 0.66]
    ]
  });

  chordCue({
    id: 'original-static-bloom', name: 'Static Bloom (Hyperpop)', bpm: 155, seed: 405,
    passes: 4,
    prog: [
      { chord: ['F3', 'A3', 'C4'], bass: 'F2' }, { chord: ['A3', 'C4', 'E4'], bass: 'A2' },
      { chord: ['D3', 'F3', 'A3', 'C4'], bass: 'D2' }, { chord: ['Bb2', 'D3', 'F3', 'Bb3'], bass: 'Bb1' }
    ],
    melody: [
      ['A4', 16, 2, 0.72], ['C5', 18, 2, 0.74], ['F5', 20, 3, 0.8], ['E5', 23, 1, 0.7],
      ['D5', 24, 2, 0.7], ['C5', 26, 2, 0.66], ['A4', 28, 4, 0.68],
      ['G4', 32, 2, 0.64], ['A4', 34, 2, 0.66], ['C5', 36, 4, 0.72], ['D5', 40, 2, 0.7],
      ['F5', 42, 2, 0.76], ['E5', 44, 4, 0.74],
      ['F5', 48, 1, 0.78], ['F4', 49, 1, 0.6], ['F5', 50, 1, 0.76], ['E5', 51, 1, 0.7],
      ['D5', 52, 2, 0.68], ['C5', 54, 2, 0.66], ['A4', 56, 2, 0.64], ['C5', 58, 2, 0.68],
      ['F4', 60, 4, 0.6]
    ]
  });

  chordCue({
    id: 'original-first-light-over-la', name: 'First Light Over LA (Pad Ballad)', bpm: 54, seed: 409,
    prog: [
      { chord: ['E2', 'B2', 'G#3', 'F#4'], bass: 'E1' }, { chord: ['C#2', 'G#2', 'E3', 'D#4'], bass: 'C#1' },
      { chord: ['A1', 'E2', 'C#3', 'B3'], bass: 'A1' }, { chord: ['B1', 'F#2', 'D#3', 'A3'], bass: 'B1' }
    ],
    melody: [
      ['G#4', 4, 3, 0.56], ['F#4', 7, 1, 0.52], ['E4', 8, 4, 0.56],
      ['C#4', 12, 2, 0.5], ['D#4', 14, 2, 0.52], ['E4', 16, 6, 0.56],
      ['B4', 24, 3, 0.62], ['G#4', 27, 1, 0.54], ['A4', 28, 4, 0.58],
      ['F#4', 32, 3, 0.54], ['G#4', 35, 5, 0.56], ['E5', 40, 4, 0.64],
      ['D#5', 44, 2, 0.58], ['B4', 46, 2, 0.56], ['C#5', 48, 6, 0.6],
      ['G#4', 56, 2, 0.52], ['E4', 58, 5, 0.52]
    ]
  });

  chordCue({
    id: 'original-solar-sail', name: 'Solar Sail (Dream Pad)', bpm: 58, seed: 413,
    prog: [
      { chord: ['Db3', 'Ab3', 'Db4', 'F4'], bass: 'Db2' }, { chord: ['Bb2', 'F3', 'Db4', 'Eb4'], bass: 'Bb1' },
      { chord: ['Gb2', 'Db3', 'Bb3', 'Eb4'], bass: 'Gb1' }, { chord: ['Ab2', 'Eb3', 'C4', 'Db4'], bass: 'Ab1' }
    ],
    melody: [
      ['F4', 3, 4, 0.52], ['Eb4', 8, 3, 0.5], ['Db4', 12, 4, 0.52],
      ['Ab4', 18, 4, 0.58], ['Bb4', 24, 3, 0.58], ['C5', 27, 5, 0.6],
      ['Db5', 34, 4, 0.62], ['C5', 40, 2, 0.56], ['Bb4', 42, 2, 0.54],
      ['Ab4', 44, 4, 0.56], ['Eb4', 50, 3, 0.5], ['F4', 54, 6, 0.52]
    ]
  });

  chordCue({
    id: 'original-rust-and-chrome', name: 'Rust and Chrome (Slow Blues)', bpm: 62, seed: 417,
    prog: [
      { chord: ['E3', 'G3', 'B3', 'F#4'], bass: 'E2' }, { chord: ['A2', 'E3', 'G3', 'B3'], bass: 'A1' },
      { chord: ['C3', 'G3', 'B3', 'E4'], bass: 'C2' }, { chord: ['B2', 'F#3', 'A3', 'D#4'], bass: 'B1' }
    ],
    melody: [
      ['B4', 4.5, 1.5, 0.62], ['G4', 6, 1, 0.56], ['E4', 7, 4, 0.6],
      ['A4', 12.5, 1, 0.58], ['G4', 13.5, 2.5, 0.56],
      ['B4', 17, 1.5, 0.62], ['D5', 18.5, 4.5, 0.68],
      ['E5', 24.5, 1, 0.68], ['D5', 25.5, 1, 0.62], ['B4', 26.5, 3.5, 0.64],
      ['G4', 36.5, 1, 0.56], ['A4', 37.5, 1.5, 0.58], ['B4', 39, 4, 0.62],
      ['E5', 44.5, 1.5, 0.68], ['D5', 46, 1, 0.62], ['B4', 47, 2.5, 0.62],
      ['A4', 52, 1, 0.56], ['G4', 53, 1.5, 0.54], ['E4', 54.5, 5, 0.58]
    ]
  });

  chordCue({
    id: 'original-violet-skyline', name: 'Violet Skyline (80s Ballad)', bpm: 78, seed: 421,
    prog: [
      { chord: ['G2', 'D3', 'G3', 'B3'], bass: 'G1' }, { chord: ['E2', 'B2', 'E3', 'G3'], bass: 'E1' },
      { chord: ['C3', 'G3', 'C4', 'E4'], bass: 'C2' }, { chord: ['D3', 'A3', 'D4', 'F#4'], bass: 'D2' }
    ],
    melody: [
      ['B4', 16, 2, 0.66], ['A4', 18, 1, 0.6], ['G4', 19, 3, 0.62],
      ['E4', 22, 2, 0.56], ['G4', 24, 3, 0.62], ['B4', 27, 1, 0.62],
      ['C5', 28, 4, 0.68], ['A4', 32, 2, 0.62], ['D5', 34, 2, 0.68],
      ['B4', 36, 4, 0.64],
      ['D5', 48, 2, 0.7], ['E5', 50, 1, 0.7], ['D5', 51, 1, 0.64],
      ['B4', 52, 2, 0.62], ['C5', 54, 2, 0.64], ['E5', 56, 3, 0.72],
      ['D5', 59, 1, 0.64], ['G4', 60, 4, 0.6]
    ]
  });

  chordCue({
    id: 'original-magnetic-north', name: 'Magnetic North (Anthem)', bpm: 100, seed: 425,
    prog: [
      { chord: ['F3', 'A3', 'C4'], bass: 'F2' }, { chord: ['Bb2', 'F3', 'Bb3', 'D4'], bass: 'Bb1' },
      { chord: ['G2', 'D3', 'Bb3', 'D4'], bass: 'G1' }, { chord: ['C3', 'G3', 'C4', 'E4'], bass: 'C2' }
    ],
    melody: [
      ['C5', 16, 3, 0.7], ['D5', 19, 1, 0.64], ['F5', 20, 4, 0.76],
      ['D5', 24, 2, 0.66], ['Bb4', 26, 2, 0.62], ['C5', 28, 4, 0.68],
      ['A4', 32, 2, 0.62], ['C5', 34, 2, 0.66], ['E5', 36, 4, 0.72],
      ['G5', 40, 3, 0.78], ['F5', 43, 1, 0.7], ['E5', 44, 2, 0.68], ['C5', 46, 2, 0.64],
      ['F5', 48, 6, 0.74], ['D5', 56, 2, 0.64], ['C5', 58, 4, 0.66]
    ]
  });

  chordCue({
    id: 'original-deep-signal', name: 'Deep Signal (Drone)', bpm: 40, seed: 429,
    passes: 1, barBeats: 12,
    prog: [
      { chord: ['F1', 'C2', 'F2'], bass: null }, { chord: ['F1', 'C2', 'Ab2'], bass: null },
      { chord: ['F1', 'C2', 'Eb2', 'G2'], bass: null }, { chord: ['F1', 'C2', 'F2', 'A2'], bass: null }
    ],
    melody: [
      ['C3', 5, 5, 0.44], ['Eb3', 13, 5, 0.46], ['Db3', 19, 4, 0.44],
      ['C3', 25, 6, 0.44], ['G3', 33, 5, 0.48], ['F3', 40, 7, 0.46]
    ]
  });

  chordCue({
    id: 'original-afterimage', name: 'Afterimage (Ambient)', bpm: 46, seed: 433,
    barBeats: 8,
    prog: [
      { chord: ['Bb2', 'F3', 'Db4', 'Eb4'], bass: 'Bb1' }, { chord: ['Gb2', 'Db3', 'Bb3', 'F4'], bass: 'Gb1' },
      { chord: ['Db3', 'Ab3', 'Db4', 'F4'], bass: 'Db2' }, { chord: ['Ab2', 'Eb3', 'C4', 'Eb4'], bass: 'Ab1' }
    ],
    melody: [
      ['F4', 3, 5, 0.48], ['Eb4', 10, 5, 0.46], ['Db4', 18, 5, 0.46],
      ['C4', 26, 5, 0.44], ['Db4', 34, 6, 0.46], ['Bb3', 42, 5, 0.44],
      ['Eb4', 50, 6, 0.48], ['F4', 58, 6, 0.48]
    ]
  });

  chordCue({
    id: 'original-late-checkout', name: 'Late Checkout (Lo-fi Keys)', bpm: 70, seed: 437,
    prog: [
      { chord: ['Bb2', 'F3', 'A3', 'D4'], bass: 'Bb1' }, { chord: ['G2', 'D3', 'F3', 'Bb3'], bass: 'G1' },
      { chord: ['Eb3', 'Bb3', 'D4', 'G4'], bass: 'Eb2' }, { chord: ['F3', 'C4', 'Eb4', 'A4'], bass: 'F2' }
    ],
    melody: [
      ['D4', 4.5, 1, 0.54], ['F4', 5.5, 0.5, 0.5], ['G4', 6, 3, 0.56],
      ['F4', 12.5, 1, 0.52], ['D4', 13.5, 2.5, 0.54],
      ['G4', 20.5, 1.5, 0.56], ['Bb4', 22, 4, 0.6],
      ['A4', 28.5, 1, 0.56], ['G4', 29.5, 1, 0.52], ['F4', 30.5, 3, 0.54],
      ['D5', 36.5, 1.5, 0.62], ['C5', 38, 2, 0.58], ['Bb4', 40.5, 2.5, 0.56],
      ['A4', 44.5, 1, 0.54], ['G4', 45.5, 1, 0.52], ['A4', 46.5, 1.5, 0.54],
      ['F4', 52.5, 1.5, 0.52], ['G4', 54, 5, 0.54]
    ]
  });

  chordCue({
    id: 'original-thunder-veil', name: 'Thunder Veil (Dark Pad)', bpm: 50, seed: 441,
    barBeats: 8,
    prog: [
      { chord: ['G2', 'D3', 'G3', 'Bb3'], bass: 'G1' }, { chord: ['Eb2', 'Bb2', 'G3', 'Bb3'], bass: 'Eb1' },
      { chord: ['C2', 'G2', 'Eb3', 'G3'], bass: 'C1' }, { chord: ['D3', 'A3', 'D4', 'F#4'], bass: 'D2' }
    ],
    melody: [
      ['D4', 4, 5, 0.5], ['Eb4', 12, 5, 0.52], ['Bb3', 20, 6, 0.48],
      ['C4', 28, 5, 0.48], ['G3', 36, 5, 0.46], ['A3', 44, 6, 0.48],
      ['D4', 52, 5, 0.5], ['G3', 58, 6, 0.46]
    ]
  });

  chordCue({
    id: 'original-warm-static', name: 'Warm Static (EP Ballad)', bpm: 64, seed: 445,
    prog: [
      { chord: ['C3', 'G3', 'B3', 'E4'], bass: 'C2' }, { chord: ['A2', 'E3', 'G3', 'C4'], bass: 'A1' },
      { chord: ['F2', 'C3', 'E3', 'A3'], bass: 'F1' }, { chord: ['G2', 'D3', 'F3', 'B3'], bass: 'G1' }
    ],
    melody: [
      ['E4', 4, 2, 0.54], ['G4', 6, 1, 0.54], ['A4', 7, 5, 0.58],
      ['G4', 14, 2, 0.54], ['E4', 16, 4, 0.54],
      ['C5', 24, 2.5, 0.62], ['B4', 26.5, 1.5, 0.58], ['A4', 28, 2, 0.56],
      ['G4', 30, 2, 0.54], ['E4', 32, 4, 0.54],
      ['D5', 40, 2, 0.62], ['E5', 42, 2, 0.64], ['C5', 44, 4, 0.6],
      ['B4', 52, 2, 0.56], ['G4', 54, 2, 0.52], ['C5', 56, 6, 0.58]
    ]
  });

  chordCue({
    id: 'original-zero-gravity-waltz', name: 'Zero Gravity Waltz', bpm: 90, seed: 449,
    barBeats: 3, passes: 4,
    prog: [
      { chord: ['D3', 'A3', 'F4'], bass: 'D2' }, { chord: ['G2', 'D3', 'Bb3'], bass: 'G1' },
      { chord: ['Bb2', 'F3', 'D4'], bass: 'Bb1' }, { chord: ['A2', 'E3', 'C#4'], bass: 'A1' }
    ],
    melody: [
      ['F4', 12, 2, 0.58], ['E4', 14, 1, 0.54], ['D4', 15, 3, 0.56],
      ['G4', 18, 2, 0.58], ['A4', 20, 1, 0.56], ['Bb4', 21, 3, 0.6],
      ['A4', 24, 2, 0.6], ['F4', 26, 1, 0.54], ['D5', 27, 3, 0.64],
      ['C#5', 30, 2, 0.6], ['E4', 32, 1, 0.5], ['A4', 33, 3, 0.58],
      ['D5', 36, 2, 0.64], ['F5', 38, 1, 0.66], ['E5', 39, 3, 0.64],
      ['D5', 42, 2, 0.62], ['Bb4', 44, 1, 0.56], ['G4', 45, 3, 0.56],
      ['A4', 48, 5, 0.58], ['F4', 54, 2, 0.52], ['D4', 56, 4, 0.54]
    ]
  });
};
...
const wave4CellCues = () => {
  cellCue({
    id: 'original-glitter-riot', name: 'Glitter Riot (Hyperpop Rage)', bpm: 170, seed: 501,
    cells: {
      R: [
        [0, 'B2', 0.8], [2, 'B2', 0.62], [4, 'D3', 0.72], [6, 'B2', 0.6],
        [8, 'F#3', 0.76], [10, 'E3', 0.62], [12, 'D3', 0.7], [14, 'C#3', 0.6]
      ],
      S: [
        [0, 'G2', 0.8], [2, 'G2', 0.62], [4, 'B2', 0.72], [6, 'G2', 0.6],
        [8, 'A2', 0.76], [10, 'B2', 0.62], [12, 'C#3', 0.7], [14, 'E3', 0.64]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: bar % 4 < 2 ? 'R' : 'S',
      bass: bar % 4 < 2 ? 'B1' : (bar % 4 === 2 ? 'G1' : 'A1'),
      bassBeats: 3.6, bassVel: 0.72
    })),
    extra: [
      ['F#5', 12, 1.5, 0.72], ['E5', 14, 2, 0.68],
      ['B4', 28, 1, 0.66], ['D5', 29.5, 2.5, 0.7],
      ['F#5', 44, 1.5, 0.74], ['G5', 46, 2, 0.76],
      ['E5', 60, 1.5, 0.7], ['F#5', 62, 2, 0.74]
    ]
  });

  cellCue({
    id: 'original-adrenaline-red', name: 'Adrenaline Red (Rage)', bpm: 160, seed: 505,
    cells: {
      A: [
        [0, 'E3', 0.8], [2, 'E3', 0.6], [4, 'G3', 0.72], [6, 'F3', 0.66],
        [8, 'E3', 0.74], [11, 'B2', 0.6], [12, 'A#2', 0.7], [14, 'B2', 0.6]
      ],
      B: [
        [0, 'E3', 0.8], [2, 'E3', 0.6], [4, 'Bb3', 0.76], [6, 'A3', 0.64],
        [8, 'G3', 0.72], [10, 'F3', 0.64], [12, 'E3', 0.72], [15, 'D3', 0.56]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: bar % 4 === 3 ? 'B' : 'A',
      bass: bar % 8 < 6 ? 'E1' : 'C1',
      bassBeats: 3.4, bassVel: 0.78
    })),
    extra: [
      ['E5', 13, 1.5, 0.72], ['F5', 15, 1, 0.7],
      ['G5', 29, 1.5, 0.74], ['F5', 31, 1, 0.68],
      ['Bb4', 45, 1.5, 0.7], ['B4', 47, 1, 0.72],
      ['E5', 61, 2.5, 0.76]
    ]
  });

  cellCue({
    id: 'original-teeth-grinder', name: 'Teeth Grinder (Rage)', bpm: 152, seed: 509,
    cells: {
      G: [
        [0, 'C#3', 0.8], [1, 'C#3', 0.6], [3, 'C#3', 0.66], [5, 'E3', 0.72],
        [8, 'D3', 0.74], [10, 'C#3', 0.62], [12, 'G3', 0.76], [14, 'F#3', 0.62]
      ],
      H: [
        [0, 'C#3', 0.8], [1, 'C#3', 0.6], [3, 'E3', 0.68], [5, 'F#3', 0.72],
        [8, 'G3', 0.78], [10, 'F#3', 0.64], [12, 'E3', 0.7], [14, 'D3', 0.6]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: bar % 2 === 0 ? 'G' : 'H',
      bass: bar % 8 < 4 ? 'C#1' : (bar % 8 < 6 ? 'A0' : 'B0'),
      bassBeats: 3.2, bassVel: 0.78
    })),
    extra: [
      ['C#5', 14, 1.5, 0.7], ['D5', 30, 1.5, 0.72],
      ['G4', 46, 1.5, 0.68], ['G#4', 47.75, 1.25, 0.7],
      ['C#5', 61, 3, 0.74]
    ]
  });

  cellCue({
    id: 'original-tokyo-monorail', name: 'Tokyo Monorail (Synthwave)', bpm: 112, seed: 513,
    cells: {
      C: [
        [0, 'C2', 0.72], [2, 'C3', 0.56], [4, 'C2', 0.66], [6, 'C3', 0.56],
        [8, 'C2', 0.72], [10, 'C3', 0.56], [12, 'C2', 0.66], [14, 'C3', 0.56]
      ],
      Ab: [
        [0, 'Ab1', 0.72], [2, 'Ab2', 0.56], [4, 'Ab1', 0.66], [6, 'Ab2', 0.56],
        [8, 'Ab1', 0.72], [10, 'Ab2', 0.56], [12, 'Ab1', 0.66], [14, 'Ab2', 0.56]
      ],
      Bb: [
        [0, 'Bb1', 0.72], [2, 'Bb2', 0.56], [4, 'Bb1', 0.66], [6, 'Bb2', 0.56],
        [8, 'Bb1', 0.72], [10, 'Bb2', 0.56], [12, 'Bb1', 0.66], [14, 'Bb2', 0.56]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: bar % 4 < 2 ? 'C' : (bar % 4 === 2 ? 'Ab' : 'Bb')
    })),
    extra: [
      ['G4', 8, 2, 0.62], ['Eb4', 10.5, 1.5, 0.58], ['F4', 12, 3, 0.6],
      ['G4', 24, 1.5, 0.62], ['Bb4', 26, 2, 0.66], ['C5', 28.5, 3.5, 0.68],
      ['Bb4', 40, 1.5, 0.62], ['G4', 42, 2, 0.6], ['F4', 44.5, 3, 0.58],
      ['Eb5', 56, 2, 0.68], ['D5', 58.5, 1.5, 0.62], ['C5', 60, 4, 0.64]
    ]
  });

  cellCue({
    id: 'original-perimeter-run', name: 'Perimeter Run (Chase Arps)', bpm: 124, seed: 517,
    cells: {
      i: [
        [0, 'F#2', 0.7], [2, 'C#3', 0.52], [4, 'F#3', 0.62], [6, 'A3', 0.54],
        [8, 'C#4', 0.66], [10, 'A3', 0.54], [12, 'F#3', 0.6], [14, 'C#3', 0.52]
      ],
      VI: [
        [0, 'D2', 0.7], [2, 'A2', 0.52], [4, 'D3', 0.62], [6, 'F#3', 0.54],
        [8, 'A3', 0.66], [10, 'F#3', 0.54], [12, 'D3', 0.6], [14, 'A2', 0.52]
      ],
      III: [
        [0, 'A2', 0.7], [2, 'E3', 0.52], [4, 'A3', 0.62], [6, 'C#4', 0.54],
        [8, 'E4', 0.66], [10, 'C#4', 0.54], [12, 'A3', 0.6], [14, 'E3', 0.52]
      ],
      VII: [
        [0, 'E2', 0.7], [2, 'B2', 0.52], [4, 'E3', 0.62], [6, 'G#3', 0.54],
        [8, 'B3', 0.66], [10, 'G#3', 0.54], [12, 'E3', 0.6], [14, 'B2', 0.52]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: ['i', 'VI', 'III', 'VII'][Math.floor(bar / 2) % 4],
      bass: ['F#1', 'D1', 'A1', 'E1'][Math.floor(bar / 2) % 4],
      bassBeats: 4, bassVel: 0.64
    })),
    extra: [
      ['C#5', 16, 3, 0.7], ['B4', 19, 1, 0.62], ['A4', 20, 4, 0.66],
      ['F#4', 24, 2, 0.6], ['G#4', 26, 2, 0.62], ['A4', 28, 4, 0.64],
      ['E5', 32, 3, 0.72], ['C#5', 35, 1, 0.64], ['D5', 36, 4, 0.68],
      ['C#5', 40, 2, 0.64], ['B4', 42, 2, 0.62], ['G#4', 44, 4, 0.62],
      ['F#5', 48, 4, 0.74], ['E5', 52, 2, 0.68], ['C#5', 54, 2, 0.64],
      ['D5', 56, 3, 0.66], ['B4', 59, 1, 0.6], ['C#5', 60, 4, 0.66]
    ]
  });

  cellCue({
    id: 'original-glass-garden', name: 'Glass Garden (Pluck Arps)', bpm: 90, seed: 521,
    stepBeats: 0.5, noteBeats: 0.45,
    cells: {
      A: [
        [0, 'A2', 0.62], [1, 'E3', 0.5], [2, 'A3', 0.56], [3, 'C#4', 0.5],
        [4, 'E4', 0.58], [5, 'C#4', 0.5], [6, 'A3', 0.54], [7, 'E3', 0.48]
      ],
      F: [
        [0, 'F#2', 0.62], [1, 'C#3', 0.5], [2, 'F#3', 0.56], [3, 'A3', 0.5],
        [4, 'C#4', 0.58], [5, 'A3', 0.5], [6, 'F#3', 0.54], [7, 'C#3', 0.48]
      ],
      D: [
        [0, 'D3', 0.62], [1, 'A3', 0.5], [2, 'D4', 0.56], [3, 'F#4', 0.5],
        [4, 'A4', 0.58], [5, 'F#4', 0.5], [6, 'D4', 0.54], [7, 'A3', 0.48]
      ],
      E: [
        [0, 'E3', 0.62], [1, 'B3', 0.5], [2, 'E4', 0.56], [3, 'G#4', 0.5],
        [4, 'B4', 0.58], [5, 'G#4', 0.5], [6, 'E4', 0.54], [7, 'B3', 0.48]
      ]
    },
    plan: Array.from({ length: 12 }, (_, bar) => ({
      cell: ['A', 'F', 'D', 'E'][bar % 4]
    })),
    extra: [
      ['C#5', 6, 2, 0.6], ['B4', 10, 2, 0.58], ['A4', 14, 2, 0.56],
      ['E5', 20, 3, 0.64], ['F#5', 26, 2, 0.64], ['E5', 30, 2, 0.6],
      ['C#5', 34, 2, 0.6], ['B4', 38, 2, 0.58], ['A4', 42, 4, 0.58]
    ]
  });

  cellCue({
    id: 'original-copper-wires', name: 'Copper Wires (Acid)', bpm: 130, seed: 525,
    cells: {
      A: [
        [0, 'G1', 0.82], [2, 'G2', 0.5], [3, 'G1', 0.6], [5, 'Bb1', 0.52],
        [6, 'G1', 0.72], [8, 'D2', 0.58], [10, 'G1', 0.66], [11, 'F2', 0.52],
        [12, 'G2', 0.76], [14, 'Db2', 0.6], [15, 'C2', 0.5]
      ],
      B: [
        [0, 'G1', 0.82], [2, 'Bb1', 0.54], [4, 'C2', 0.68], [6, 'Db2', 0.6],
        [7, 'D2', 0.74], [9, 'G1', 0.52], [10, 'D2', 0.64], [12, 'F2', 0.72],
        [13, 'G2', 0.56], [14, 'F2', 0.6], [15, 'D2', 0.52]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: (bar >> 2) % 2 === 0 ? 'A' : 'B',
      bass: 'G0', bassBeats: 0.3, bassVel: 0.68
    }))
  });

  cellCue({
    id: 'original-dust-devils', name: 'Dust Devils (Sequence)', bpm: 118, seed: 529,
    cells: {
      D: [
        [0, 'D3', 0.72], [3, 'A3', 0.52], [6, 'D4', 0.64], [8, 'C4', 0.52],
        [10, 'A3', 0.56], [12, 'F3', 0.6], [14, 'E3', 0.48]
      ],
      Bb: [
        [0, 'Bb2', 0.72], [3, 'F3', 0.52], [6, 'Bb3', 0.64], [8, 'A3', 0.52],
        [10, 'F3', 0.56], [12, 'D3', 0.6], [15, 'F3', 0.48]
      ],
      C: [
        [0, 'C3', 0.72], [3, 'G3', 0.52], [6, 'C4', 0.64], [8, 'Bb3', 0.52],
        [10, 'G3', 0.56], [12, 'E3', 0.6], [14, 'D3', 0.48]
      ]
    },
    plan: Array.from({ length: 18 }, (_, bar) => ({
      cell: ['D', 'D', 'Bb', 'Bb', 'C', 'D'][bar % 6],
      bass: ['D2', 'D2', 'Bb1', 'Bb1', 'C2', 'D2'][bar % 6],
      bassBeats: 4, bassVel: 0.6
    })),
    extra: [
      ['A4', 24, 4, 0.56], ['F4', 32, 4, 0.52], ['G4', 40, 4, 0.54],
      ['D5', 48, 4, 0.6], ['C5', 56, 4, 0.56], ['A4', 64, 6, 0.54]
    ]
  });

  cellCue({
    id: 'original-halogen-heart', name: 'Halogen Heart (Hyperpop Pluck)', bpm: 158, seed: 533,
    cells: {
      D: [
        [0, 'D3', 0.66], [2, 'A3', 0.52], [4, 'D4', 0.6], [6, 'F#4', 0.52],
        [8, 'A4', 0.62], [10, 'F#4', 0.52], [12, 'D4', 0.58], [14, 'A3', 0.5]
      ],
      Bm: [
        [0, 'B2', 0.66], [2, 'F#3', 0.52], [4, 'B3', 0.6], [6, 'D4', 0.52],
        [8, 'F#4', 0.62], [10, 'D4', 0.52], [12, 'B3', 0.58], [14, 'F#3', 0.5]
      ],
      G: [
        [0, 'G2', 0.66], [2, 'D3', 0.52], [4, 'G3', 0.6], [6, 'B3', 0.52],
        [8, 'D4', 0.62], [10, 'B3', 0.52], [12, 'G3', 0.58], [14, 'D3', 0.5]
      ],
      A: [
        [0, 'A2', 0.66], [2, 'E3', 0.52], [4, 'A3', 0.6], [6, 'C#4', 0.52],
        [8, 'E4', 0.62], [10, 'C#4', 0.52], [12, 'A3', 0.58], [14, 'E3', 0.5]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: ['D', 'Bm', 'G', 'A'][bar % 4],
      bass: ['D1', 'B0', 'G0', 'A0'][bar % 4],
      bassBeats: 3.8, bassVel: 0.62
    })),
    extra: [
      ['F#5', 16, 1, 0.74], ['A5', 17, 1, 0.76], ['F#5', 18, 1.5, 0.7],
      ['E5', 19.5, 0.5, 0.64], ['D5', 20, 2, 0.68], ['E5', 22, 2, 0.68],
      ['F#5', 24, 1, 0.72], ['F#4', 25, 0.5, 0.56], ['F#5', 25.5, 0.5, 0.7],
      ['G5', 26, 1.5, 0.72], ['E5', 27.5, 0.5, 0.64], ['A5', 28, 4, 0.78],
      ['B5', 48, 1.5, 0.8], ['A5', 49.5, 0.5, 0.7], ['F#5', 50, 1, 0.7],
      ['D5', 51, 1, 0.64], ['E5', 52, 2.5, 0.68], ['C#5', 54.5, 1.5, 0.62],
      ['D5', 56, 4, 0.7]
    ]
  });

  cellCue({
    id: 'original-crystal-run', name: 'Crystal Run (Bright Arps)', bpm: 132, seed: 537,
    cells: {
      Em: [
        [0, 'E3', 0.66], [2, 'B3', 0.5], [4, 'E4', 0.6], [6, 'G4', 0.52],
        [8, 'B4', 0.64], [10, 'G4', 0.52], [12, 'E4', 0.58], [14, 'B3', 0.5]
      ],
      C: [
        [0, 'C3', 0.66], [2, 'G3', 0.5], [4, 'C4', 0.6], [6, 'E4', 0.52],
        [8, 'G4', 0.64], [10, 'E4', 0.52], [12, 'C4', 0.58], [14, 'G3', 0.5]
      ],
      G: [
        [0, 'G2', 0.66], [2, 'D3', 0.5], [4, 'G3', 0.6], [6, 'B3', 0.52],
        [8, 'D4', 0.64], [10, 'B3', 0.52], [12, 'G3', 0.58], [14, 'D3', 0.5]
      ],
      D: [
        [0, 'D3', 0.66], [2, 'A3', 0.5], [4, 'D4', 0.6], [6, 'F#4', 0.52],
        [8, 'A4', 0.64], [10, 'F#4', 0.52], [12, 'D4', 0.58], [14, 'A3', 0.5]
      ]
    },
    plan: Array.from({ length: 16 }, (_, bar) => ({
      cell: ['Em', 'C', 'G', 'D'][bar % 4],
      bass: ['E1', 'C1', 'G1', 'D1'][bar % 4],
      bassBeats: 4, bassVel: 0.6
    })),
    extra: [
      ['B4', 16, 2, 0.64], ['G4', 18, 2, 0.6], ['A4', 20, 2, 0.62], ['F#4', 22, 2, 0.58],
      ['E5', 32, 3, 0.7], ['D5', 35, 1, 0.62], ['C5', 36, 2, 0.64], ['B4', 38, 2, 0.6],
      ['G5', 48, 3, 0.74], ['F#5', 51, 1, 0.66], ['E5', 52, 4, 0.68],
      ['D5', 56, 2, 0.62], ['B4', 58, 2, 0.6], ['E5', 60, 4, 0.66]
    ]
  });
};

/**
 * Midnight Slide — trap cue with long overlapping 808 slides in F# minor.
 */
const midnightSlide = () => {
  const bpm = 135;
  const sub = new Part({ bpm, seed: 541 });
  const keys = new Part({ bpm, humanize: 0.012, seed: 542 });

  const SLIDES = [
    ['F#1', 0, 3.4, 0.85], ['A1', 3.6, 2.2, 0.76], ['C#2', 6, 2.2, 0.74],
    ['F#1', 8, 3.4, 0.85], ['E1', 11.6, 0.6, 0.62], ['D1', 12, 4.4, 0.8],
    ['C#1', 16, 4.4, 0.78], ['B0', 20, 4.4, 0.76],
    ['F#1', 24, 3.4, 0.85], ['G#1', 27.6, 0.6, 0.6], ['A1', 28, 2.2, 0.76],
    ['C#2', 30, 2.4, 0.74]
  ];
  for (let round = 0; round < 3; round++) {
    SLIDES.forEach(([note, at, beats, vel]) => sub.add(note, round * 32 + at, beats, vel));
  }

  const KEYS = [
    ['C#4', 2, 1.5, 0.56], ['A3', 4, 2, 0.52], ['G#3', 7, 1.5, 0.5],
    ['F#4', 10, 2, 0.58], ['E4', 13.5, 1.5, 0.54],
    ['C#4', 18, 2, 0.54], ['D4', 21.5, 2, 0.56],
    ['C#5', 26, 1.5, 0.62], ['B4', 28, 1.5, 0.58], ['A4', 30, 2, 0.56],
    ['F#4', 34, 2, 0.56], ['G#4', 38, 2, 0.54], ['A4', 42, 2, 0.58],
    ['E5', 50, 2, 0.64], ['D5', 53.5, 1.5, 0.58], ['C#5', 56, 3, 0.6],
    ['B4', 61, 2, 0.56], ['A4', 66, 2, 0.54], ['G#4', 70, 2, 0.52],
    ['F#4', 74, 4, 0.54], ['C#4', 82, 2, 0.5], ['F#3', 88, 4, 0.48]
  ];
  KEYS.forEach(([note, at, beats, vel]) => keys.add(note, at, beats, vel));

  writeMidi({
    id: 'original-midnight-slide',
    name: 'Midnight Slide (Trap)',
    bpm,
    parts: [sub, keys]
  });
};
...
/**
 * Black Ice — colder trap cue in D minor, sparse and heavy.
 */
const blackIce = () => {
  const bpm = 145;
  const sub = new Part({ bpm, seed: 545 });
  const bells = new Part({ bpm, humanize: 0.015, seed: 546 });

  const PATTERN = [
    ['D1', 0, 2.6, 0.84], ['D1', 3, 0.8, 0.62],
    ['F1', 4, 2.6, 0.78], ['A0', 8, 3.6, 0.8],
    ['C1', 12, 2.6, 0.76], ['C#1', 15, 0.8, 0.6],
    ['D1', 16, 2.6, 0.84], ['G0', 20, 3.6, 0.78],
    ['A0', 24, 2.6, 0.78], ['Bb0', 27, 0.8, 0.6], ['A0', 28, 3.6, 0.78]
  ];
  for (let round = 0; round < 3; round++) {
    PATTERN.forEach(([note, at, beats, vel]) => sub.add(note, round * 32 + at, beats, vel));
  }

  const BELLS = [
    ['D5', 1, 1, 0.6], ['A4', 2.5, 1.5, 0.54], ['F4', 5, 2, 0.52],
    ['G#4', 9, 1.5, 0.58],
    ['A4', 13, 1, 0.54], ['C5', 14.5, 1.5, 0.58],
    ['D5', 17, 1.5, 0.6], ['E5', 20.5, 1.5, 0.62], ['D5', 23, 1, 0.56],
    ['F5', 25, 2, 0.64], ['E5', 28, 1.5, 0.58], ['C#5', 30, 2, 0.56],
    ['D5', 33, 1, 0.6], ['A4', 34.5, 1.5, 0.54], ['F4', 37, 2, 0.52],
    ['G#4', 41, 1.5, 0.58],
    ['A5', 49, 1.5, 0.66], ['G5', 52.5, 1.5, 0.6], ['F5', 56, 2, 0.6],
    ['E5', 60.5, 1.5, 0.56], ['D5', 65, 2, 0.56], ['A4', 70, 2, 0.52],
    ['D4', 78, 4, 0.5]
  ];
  BELLS.forEach(([note, at, beats, vel]) => bells.add(note, at, beats, vel));

  writeMidi({
    id: 'original-black-ice',
    name: 'Black Ice (Trap)',
    bpm,
    parts: [sub, bells]
  });
